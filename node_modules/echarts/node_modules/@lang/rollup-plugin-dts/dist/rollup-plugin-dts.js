...
class NamespaceFixer {
    constructor(sourceFile, allNamespacesFixers) {
        this.sourceFile = sourceFile;
        this.allNamespacesFixers = allNamespacesFixers;
    }
    findNamespaces() {
        const namespaces = [];
        const items = {};
        ...
        for (const node of this.sourceFile.statements) {
            ...
            if (ts__default['default'].isClassDeclaration(node)) {
            ...
            else if (ts__default['default'].isNamedExports(node)) {
                // Reversed map
                ...
                if (node.exportClause && ts__default['default'].isNamedExports(node.exportClause)) {
                    // Reversed map
...
function renderTypeParams(code, _items, typeParameters) {
    if (!typeParameters || !typeParameters.length) {
        return { in: '', out: '' };
    }
    function renderSingleParam(paramNode, _idx) {
...
const plugin = (options = {}) => {
    ...
    const allNamespacesFixers = new Map();
